...
export const giveFeedbackSchema = z.object({
  ...
  schoolYear: z
    .number()
    .min(2020, 'Invalid school year')
    .max(3050, 'Invalid school year'),
  ...
  rating: z
    .number()
    .min(1, 'Please rate the course')
    .max(5, 'Rating cannot exceed 5'),
  workloadRating: z
    .number()
    .min(1, 'Please rate the workload')
    .max(5, 'Rating cannot exceed 5'),
...
export function validateFormData(
  ...
  | { success: true; data: GiveFeedbackFormData }
